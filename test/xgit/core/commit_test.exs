defmodule Xgit.Core.CommitTest do
  ...
  describe "from_object/1" do
    ...
    # defp write_git_commit_and_read_back(ref_commit_fn) do
    #   %{xgit_path: path, xgit_repo: repo} = context = OnDiskRepoTestCase.repo!()

    #   commit_id = ref_commit_fn.(path)

    #   assert {:ok, %Object{} = object} = Repository.get_object(repo, commit_id)
    #   assert {:ok, %Commit{} = commit} = Commit.from_object(object)

    #   Map.put(context, :commit, commit)
    # end
