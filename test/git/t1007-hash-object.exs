defmodule Xgit.T1007_Basic do
  ...
  describe "git hash-object" do
    defp echo_without_newline(content, path) do
      File.write!(path, content)
    end

    defp assert_blob_does_not_exist(repo, object_id) do
      assert {:error, :not_found} = Plumbing.cat_file(repo, object_id)
    end

    defp assert_blob_exists(repo, object_id) do
      assert {:ok, _} = Plumbing.cat_file(repo, object_id)
    end

    @hello_content "Hello World"
    @example_content "This is an example"

    defp setup_repo(repo_path) do
      echo_without_newline(@hello_content, Path.join(repo_path, "hello"))
      echo_without_newline(@example_content, Path.join(repo_path, "example"))
    end
    ...
    # assert_blob_does_not_exist "$(test_oid hello)"

    # test_expect_success 'hash from stdin' '
    # 	test "$(test_oid example)" = $(git hash-object --stdin < example)
    # '

    # assert_blob_does_not_exist "$(test_oid example)"

    # test_expect_success 'hash a file and write to database' '
    # 	test "$(test_oid hello)" = $(git hash-object -w hello)
    # '

    # assert_blob_exists "$(test_oid hello)"
    ...
    # 	assert_blob_exists "$(test_oid example)"
